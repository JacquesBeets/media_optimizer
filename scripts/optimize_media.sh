...
process_file() {
    ...
    if [ "$codec" = "hevc" ]; then
        ...
        ffmpeg -loglevel debug -i "$input_file" -map 0:v:0 -map 0:m:language:eng -metadata:s:a language=eng -c:v copy -c:a ac3 -b:a 384k -f mp4 -movflags +faststart "$temp_output"
        # ffmpeg -nostdin -y \
